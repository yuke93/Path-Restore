...
class DynamicModelTest(DynamicModel):
    ...
    def test_whole_image(self):
        ...
        for noi_level in noise_levels:
            # save folder
            if is_save:
                save_folder_cur = save_folder + 'noise{}/'.format(noi_level)
            ...
            for key in test_label.keys():
                ...
                # run
                patch_in, pos, count_map = whole2patch(
                    im_data,
                    (data_size, data_size),
                    (stride, stride)
                )
                feed_dict = {self.input_ph_dyn: patch_in,
                             self.is_train_ph: False}
                patch_len = len(patch_in)
                if limit_batch < 0 or limit_batch >= patch_in:
                    patch_out, *actions = self.sess_dyn.run(
                        [self.output_dyn] + self.test_info[:-1],
                        feed_dict=feed_dict
                    )
                else:
                    ...
                    for idx in range(0, patch_len, limit_batch):
                        patch_cur = patch_in[
                            idx: min(idx + limit_batch, patch_len),
                            ...
                        ]
                        feed_dict = {self.input_ph_dyn: patch_cur,
                                     self.is_train_ph: False}
                        patch_out_cur, *actions_cur = self.sess_dyn.run(
                            [self.output_dyn] + self.test_info[:-1],
                            feed_dict=feed_dict
                        )
                ...
                im_output = patch2whole(patch_out, pos, count_map,
                                        (stride, stride))
                ...
                # save image
                if is_save:
                    cv2.imwrite(save_folder_cur + key + '.png',
                                im_output[:, :, ::-1] * 255)
